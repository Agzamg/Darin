...
from renju_ import run, run_test, Player, Game
from agent_ import BackendAgent,HumanAgent, Agent
...
class Runner(object):
    ...
    def __init__(self, black, white):
        self.game = Game()
        self.papa_black = load_model('Agzblack.h5')
        self.papa_black_graph = tf.get_default_graph()
        self.papa_white = load_model('Agzwhite.h5')
        ...
        if (black == 'MCTS'):
            ...
            self.black = RandomBeamSearchAgent(color = 'black', name = 'TreeBlack', 
                   black_model = (self.papa_black, self.papa_black_graph), 
                   white_model = (self.papa_white, self.papa_white_graph), 
                   timeout = 2.75, high = 16, gamma = 0.99, fine = 1.0, bonus = 1.0, verbose = 0)
        else:
           self.black = 'Human'
        ...
        if (white == 'MCTS'):
            self.white = MCTS(name = 'MCTSSLOW', 
                   ...
                   black_rollout = (self.papa_black, self.papa_black_graph), 
                   white_rollout = (self.papa_white, self.papa_white_graph), 
                   timeout = 2.75, high = 14, gamma = 0.99, verbose = 0, min_prob = 0.8, param1 = 0.2, param2 = 0.65)
        elif (white == 'Policy'):
            ...
            self.white = RandomBeamSearchAgent(color = 'white', name = 'TreeWlack', 
                   black_model = (self.papa_black, self.papa_black_graph), 
                   white_model = (self.papa_white, self.papa_white_graph), 
                   timeout = 3, high = 16, gamma = 0.99, fine = 1.0, bonus = 1.0, verbose = 0)
        else:
            self.white = 'Human'
